...
export default class CustomList extends Component {
  ...
  openList = (ev) => {
    if (
      !$(ev.target).parents(".list-container").hasClass("disabled") &&
      !$(ev.target).hasClass("disabled")
    ) {
    ...
    if (this.props.list) {
      if (this.props.list !== "custom") {
        ...
        for (let i = 0; i < this.lists[this.props.list].length; i++) {
          if (
            this.modifyLists.indexOf(this.props.list) >= 0 &&
            !this.props.all &&
            i === 0
          ) {
            continue;
          ...
          tmepList.push(
            <div
              key={i}
              className="option"
              onClick={(ev) => this.selecting(ev, choice)}
            >
              {choice}
            </div>
          );
        ...
        for (let i = 0; i < this.props.items.length; i++) {
          ...
          tmepList.push(
            <div
              key={i}
              datatype={
                Array.isArray(this.props.items[i]) ? this.props.items[i][1] : ""
              }
              className="option"
              onClick={(ev) => this.selecting(ev, choice)}
            >
          ...
          if (i === this.props.items.length - 1) {
            this.setState({
              list: [...tmepList],
              selected:
                this.props.selected ?? Array.isArray(this.props.items[i])
    ...
    return (
      ...
      >
        <div className="selected" onClick={this.openList}>
          ...
          <div className="name" key={this.state.selected}>
            {this.state.selected}
          </div>
        </div>
