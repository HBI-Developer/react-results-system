...
export default connect(fetchData)(class StatisticsPage extends Component {
  nothing = (
  ...
  computedMidiumForSubjects = (ev, state = 'الكل') => {
    let studentsList = (state !== 'الكل') ? this.props.students.filter(student => student.state === state) : this.props.students,
      males = [],
      females = [],
      degrees = [],
    ...
    if (studentsList.length) {
      ...
      this.props.subjects.map(i => {
        ...
        males.map(j => {
          let studentDegrees = this.props.degrees.filter(degree => degree.ssn === j)[0];
  
          if (studentDegrees) {
            let theDegree = studentDegrees.degrees.filter(degree => degree[0] === i.id)[0];
        ...
        degrees[degrees.length - 1].push(finalMalesDegree ? (finalMalesDegree / malesNumber).toFixed(2) : 0);
        degrees[degrees.length - 1].push(finalFemalesDegree ? (finalFemalesDegree / femalesNumber).toFixed(2) : 0);

        let bothDegree = 0;

        if (finalMalesDegree && finalFemalesDegree) {
          bothDegree = ((parseFloat(degrees[degrees.length - 1][1]) + parseFloat(degrees[degrees.length - 1][2])) / 2).toFixed(2);
      ...
      degrees.map(i => {
        ...
        tableTemplate.push(<SubjectStatistic
      ...
      tableTemplate.push(<div key={'nothing'} className="nothing">لا توجد إحصائيات لعرضها هنا لعدم وجود الدرجات.</div>);
    }
    ...
    this.setState({
      template: (
        <div className="subjects-statistics">
          ...
          <div className="statistics-table">
            ...
            <CustomList className={'states'} list={"states"} all={true} selecting={this.computedMidiumForSubjects} />
            {tableTemplate}
          </div>
      ...
      finallyDegrees: [finalStatistics[0], finalStatistics[1], finalStatistics[2]]
    });
