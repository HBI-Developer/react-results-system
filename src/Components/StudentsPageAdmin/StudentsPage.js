...
export default connect(
  ...
  class StudentsPage extends Component {
        ...
        setTimeout(() => {
          ...
          for (let i = 0; ; i++) {
            ...
            temp.push(
              <Student
                key={"student_" + i}
                clicking={this.expendedStudent}
                removing={this.removeStudentFromSystem}
                editing={this.editPanel}
                info={this.state.students[i]}
              />
            );
          }
    ...
    updateStudentsTable = (ev, action) => {
      if (ev.target.classList.value.indexOf("disabled") === -1) {
        ...
        for (let i = this.state.limit * (page - 1); ; i++) {
          if (
            i >= this.state.limit * page ||
            i === this.state.students.length
          ) {
            break;
          }
    ...
    changeState = (ev, st) => {
      let newStudents =
        st !== "الكل"
          ? this.props.students.filter((student) => student.state === st)
          : this.props.students;
    ...
    addNewStudent = (_) => {
      ...
      if (ssn && (ssn < 1000000000 || ssn > 9999999999)) {
        $(".inputs .ssn")
          ...
          .next()
          .text("يجب أن يكون الرقم الوطني مكون من 10 أرقام فقط.")
          .css("display", "block");
      }

      if (sittingNumber && (sittingNumber < 10000 || sittingNumber > 99999)) {
        $(".inputs .sitting-number")
          .css("border-color", "#f44336")
          .next()
          .text("يجب أن يكون رقم الجلوس مكون من 5 أرقام فقط.")
          .css("display", "block");
      }
      ...
      if (this.props.students.filter((student) => student.ssn === ssn).length) {
        $(".inputs .ssn")
          .css("border-color", "#f44336")
          .next()
          .text("هذا الرقم الوطني موجودٌ مسبقاً.")
          .css("display", "block");
      }

      if (
        this.props.students.filter(
          (student) => student.sittingNumber === sittingNumber
        ).length
      ) {
        $(".inputs .sitting-number")
          .css("border-color", "#f44336")
          .next()
          .text("رقم الجلوس هذا موجودٌ مسبقاً.")
          .css("display", "block");
      }

      if (!$(".inputs .error[style]").length) {
        let name = `${$(".inputs .first-name").val().trim()} ${$(
            ".inputs .middle-name"
          )
            .val()
            .trim()} ${$(".inputs .last-name").val().trim()}`,
          age = +$(".inputs .age").val(),
    ...
    editPanel = (ev) => {
      clearInputs();
      let currentStudent = this.props.students.filter(
        (student) =>
          student.ssn ===
          +$(ev.target)
            .parents(".info-name")
    ...
    editNow = (_) => {
      ...
      let studentSittingNumber = this.props.students.filter(
          (student) => student.ssn === this.state.currentStudent
        )[0].sittingNumber,
        sittingNumber = +$(".inputs .sitting-number").val();
      ...
      if (
        ...
        sittingNumber !== studentSittingNumber
      ) {
        $(".inputs .sitting-number")
          .css("border-color", "#f44336")
          .next()
      ...
      if (!$(".inputs .error[style]").length) {
        ...
        if (
          ...
          this.props.degrees.filter(
            (degree) => degree.ssn === this.state.currentStudent
          ).length
        ) {
          let subjects = [],
            allowMajors = ["مشترك", major],
            currentStudentDegrees = this.props.degrees.filter(
              (degree) => degree.ssn === this.state.currentStudent
            )[0].degrees;

          subjects = this.props.subjects.filter(
            (subject) => allowMajors.indexOf(subject.major) > -1
          );
        ...
        <div className="students-page">
          ...
          <div className="container">
            ...
            <div className="students-table" key={"students_" + this.state.page}>
              ...
              <CustomList
                key={"GS_" + this.state.currentGenetalState}
                className={"states"}
                list={"states"}
                all={true}
                selected={this.state.currentGenetalState}
              ...
              <TablesPagination
                items={this.state.students.length}
                current={this.state.page}
                limit={this.state.limit}
                clicking={this.updateStudentsTable}
              />
          ...
          <div className="confirm">
            ...
            <div className="body">
              ...
              <div className="buttons">
                <div className="yes" onClick={this.removingNow}>
                  نعم
                </div>
                <div
                  className="no"
          ...
          <div className="cover">
            <div className="student-inputs">
              ...
              <div className="inputs">
                ...
                <CustomList
                  key={"1" + this.state.currentGender}
                  className={"gender"}
                  selected={this.state.currentGender}
                  list={"genders"}
                />
